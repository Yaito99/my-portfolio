...
function Bar(props){
	...
	return(
			<div className="skill-bar row" onAnimationEnd={() =>{setAnimeStateA(true)}}>
				<Col className={animestateA ? "bar-percent bar-animation" : "bar-percent"} lg={10} style={{maxWidth: perc }}><div className="bar-name">{props.barname}</div></Col>
				<Col className="d-flex flex-row-reverse">{perc}</Col>
			</div>)
}
